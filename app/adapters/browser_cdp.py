...
def launch_edge_with_cdp(extra_args: Optional[list] = None) -> subprocess.Popen:
    ...
    Notes:
    - We build the profile path in Python to avoid backslash-escape problems.
    - We pass a proper list to subprocess with shell=False (safer and cleaner).
    """
    ...
    # Launch without using the shell; returns immediately
    try:
        return subprocess.Popen(args, shell=False)
    except FileNotFoundError:
        # Edge might not be on PATH on some machines; try a common install path
        edge_path = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
        if os.path.exists(edge_path):
            args[0] = edge_path
            return subprocess.Popen(args, shell=False)
        # Last-resort fallback: try Chrome if Edge truly isn't found
        args[0] = "chrome.exe"
        return subprocess.Popen(args, shell=False)
...
async def _get_ws_debugger_url() -> Optional[str]:
    """
    Ask the local CDP /json/version endpoint for the websocket URL that
    lets us send CDP commands.
    """
...
async def open_url(url: str) -> bool:
    ...
    try:
        async with websockets.connect(ws) as conn:
            ...
            # Create a new tab (target) with the desired URL
            await conn.send(json.dumps({
                "id": 2,
