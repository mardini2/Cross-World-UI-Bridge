...
class AsyncSocket(base_socket.BaseSocket):
    ...
    async def receive(self, pkt):
        ...
        if pkt.packet_type == packet.PONG:
            ...
            await self.close(
                wait=False, abort=True, reason=self.server.reason.CLIENT_DISCONNECT
            )
        else:
    ...
    async def check_ping_timeout(self):
        ...
        if self.last_ping and time.time() - self.last_ping > self.server.ping_timeout:
            ...
            await self.close(
                wait=False, abort=False, reason=self.server.reason.PING_TIMEOUT
            )
            return False
        return True
    ...
    async def _websocket_handler(self, ws):
        ...
        await self.close(
            wait=False, abort=True, reason=self.server.reason.TRANSPORT_CLOSE
        )
