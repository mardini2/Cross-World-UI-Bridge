...
def _collect_graphviz_files():
    ...
    for program_name in progs:
        ...
        logger.debug(
            "hook-pygraphviz: collecting graphviz program %r: %r",
            program_name,
            program_binary,
        )
    ...
    if compat.is_win:
        ...
        if hasattr(bindepend, "get_imports"):
            ...
            dot_imports = [
                path
                for name, path in bindepend.get_imports(dot_binary)
                if path is not None
            ]
        else:
            # PyInstaller < 6.0
        ...
        graphviz_lib_paths = [
            ...
            if any(
                candidate in os.path.basename(path)
                for candidate in graphviz_lib_candidates
            )
        ]
        ...
        logger.debug(
            "hook-pygraphviz: location of graphviz shared libraries: %r",
            str(graphviz_lib_dir),
        )
    ...
    if not plugin_dir.is_dir():
        logger.warning(
            "hook-pygraphviz: could not determine location of graphviz plugins!"
        )
        return binaries, datas

    logger.info(
        "hook-pygraphviz: collecting graphviz plugins from directory: %r",
        str(plugin_dir),
    )

    binaries += [
        (str(file), plugin_dest_dir) for file in plugin_dir.glob(plugin_pattern)
    ]
    datas += [
