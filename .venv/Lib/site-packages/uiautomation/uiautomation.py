...
from typing import (  # need 'pip install typing' for Python3.4
    Any,
    Callable,
    Dict,
    Generator,
...
class Bitmap:
    ...
    def FromHandle(
        ...
        cbmp = _DllClient.instance().dll.BitmapFromWindow(
            ctypes.c_size_t(root.NativeWindowHandle),
            left,
            top,
            right,
            bottom,
    ...
    def _FromGdiplusBitmap(cbmp: int) -> "Bitmap":
        ...
        _DllClient.instance().dll.BitmapGetRawFormat(
            ctypes.c_size_t(cbmp), ctypes.byref(formatType)
        )
        if formatType.value == RawFormat.JPEG:
            ...
            bitmap = GIF()
            bitmap._bitmap = cbmp
            bitmap._frameCount = _DllClient.instance().dll.MultiBitmapGetFrameCount(
                ctypes.c_size_t(cbmp)
            )
            bitmap._GetGifDealy()
    ...
    def FromBytes(
        ...
        if format is not None:
            ...
            return bitmap
        cbmp = _DllClient.instance().dll.BitmapFromBytes(
            ctypes.c_char_p(data), len(data), 0
        )
        return Bitmap._FromGdiplusBitmap(cbmp)
    ...
    def ToFile(self, savePath: str, *, quality: int = None) -> bool:
        ...
        gdiStatus = _DllClient.instance().dll.BitmapToFile(
            ctypes.c_size_t(self._bitmap),
    ...
    def ToPILImage(self) -> "PIL.Image.Image":
        ...
        return Image.frombytes(
            "RGBA", (self.Width, self.Height), self.ToBytes("BGRA"), "raw", "BGRA"
        )
    ...
    def GetRawFormat(self) -> int:
        ...
        if self._format == RawFormat.Undefined:
            formatType = ctypes.c_uint()
            _DllClient.instance().dll.BitmapGetRawFormat(
                ctypes.c_size_t(self._bitmap), ctypes.byref(formatType)
            )
    ...
    def SetPixelColor(self, x: int, y: int, argb: int) -> bool:
        ...
        Return bool, True if succeed otherwise False.
        """
        gdiStatus = _DllClient.instance().dll.BitmapSetPixel(
    ...
    def GetPixelColorsHorizontally(self, x: int, y: int, count: int) -> ctypes.Array:
        ...
        values = arrayType()
        gdiStatus = _DllClient.instance().dll.BitmapGetPixelsHorizontally(
    ...
    def SetPixelColorsHorizontally(self, x: int, y: int, colors: Sequence[int]) -> bool:
        ...
        gdiStatus = _DllClient.instance().dll.BitmapSetPixelsHorizontally(
    ...
    def GetPixelColorsVertically(self, x: int, y: int, count: int) -> ctypes.Array:
        ...
        arrayType = ctypes.c_uint32 * count
        values = arrayType()
        gdiStatus = _DllClient.instance().dll.BitmapGetPixelsVertically(
            ctypes.c_size_t(self._bitmap), x, y, values, count
        )
        return values
    ...
    def SetPixelColorsVertically(self, x: int, y: int, colors: Sequence[int]) -> bool:
        ...
        if not isinstance(colors, ctypes.Array):
            arrayType = ctypes.c_uint32 * count
            colors = arrayType(*colors)
        gdiStatus = _DllClient.instance().dll.BitmapSetPixelsVertically(
            ctypes.c_size_t(self._bitmap), x, y, colors, count
        )
        return gdiStatus == 0
    ...
    def GetPixelColorsOfRect(
        ...
        values = arrayType()
        gdiStatus = _DllClient.instance().dll.BitmapGetPixelsOfRect(
            ctypes.c_size_t(self._bitmap), x, y, width, height, values
        )
        return values

    def SetPixelColorsOfRect(
        self, x: int, y: int, width: int, height: int, colors: Sequence[int]
    ) -> bool:
        """
        x: int.
        y: int.
        ...
        if not isinstance(colors, ctypes.Array):
            arrayType = ctypes.c_uint32 * (width * height)
            colors = arrayType(*colors)
        gdiStatus = _DllClient.instance().dll.BitmapSetPixelsOfRect(
    ...
    def Clone(self) -> "MemoryBMP":
        ...
        """
        cbmp = _DllClient.instance().dll.BitmapClone(
            ctypes.c_size_t(self._bitmap), 0, 0, self._width, self._height
        )
        return Bitmap._FromGdiplusBitmap(cbmp)
    ...
    def Resize(self, width: int, height: int) -> "MemoryBMP":
        ...
        """
        cbmp = _DllClient.instance().dll.BitmapResize(
    ...
    def Rotate(self, angle: float, backgroundColor: int = 0xFFFFFFFF) -> "MemoryBMP":
        ...
        """
        cbmp = _DllClient.instance().dll.BitmapRotate(
            ctypes.c_size_t(self._bitmap), ctypes.c_float(angle), backgroundColor
        )
        return Bitmap._FromGdiplusBitmap(cbmp)
    ...
    def RotateFlip(self, rotateFlip: int) -> "MemoryBMP":
        ...
        """
        bitmap = self.Copy()
        gdiStatus = _DllClient.instance().dll.BitmapRotateFlip(
            ctypes.c_size_t(bitmap._bitmap), rotateFlip
        )
        bitmap._GetSize()
        return bitmap

    def RotateWithSameSize(
        self, dx: float, dy: float, angle: float, backgroundColor: int = 0xFFFFFFFF
    ) -> "MemoryBMP":
        """
        ...
        Return a new `MemoryBMP`, the original is not modified.
        """
        cbmp = _DllClient.instance().dll.BitmapRotateWithSameSize(
            ctypes.c_size_t(self._bitmap),
            ctypes.c_float(dx),
            ctypes.c_float(dy),
    ...
    def __repr__(self) -> str:
        return "<{}(Width={}, Height={}) at 0x{:X}>".format(
            self.__class__.__name__, self._width, self._height, id(self)
        )
...
class MultiFrameBitmap(Bitmap):
    ...
    def SelectActiveFrame(self, index: int) -> bool:
        gdiStatus = _DllClient.instance().dll.MultiBitmapSelectActiveFrame(
            ctypes.c_size_t(self._bitmap), index
        )
        return gdiStatus == 0
    ...
    def __str__(self) -> str:
        return "{}(Width={}, Height={}, FrameCount={})".format(
            self.__class__.__name__, self._width, self._height, self._frameCount
        )

    def __repr__(self) -> str:
        return "<{}(Width={}, Height={}, FrameCount={}) at 0x{:X}>".format(
            self.__class__.__name__,
            self._width,
            self._height,
            self._frameCount,
...
class TIFF(MultiFrameBitmap):
    ...
    def ToTiffFile(path: str, bitmaps: List["Bitmap"]) -> bool:
        ...
        gdiStatus = _DllClient.instance().dll.MultiBitmapToFile(
            cbitmaps, None, len(bitmaps), ctypes.c_wchar_p(path), "image/tiff"
        )
        return gdiStatus == 0
...
class GIF(MultiFrameBitmap):
    ...
    def _GetGifDealy(self):
        if self._bitmap:
            delayDataSize = _DllClient.instance().dll.MultiBitmapGetFrameDelaySize(
                ctypes.c_size_t(self._bitmap)
            )
            delayData = (ctypes.c_byte * delayDataSize)()
            valueOffset = ctypes.c_int()
            gdiStatus = _DllClient.instance().dll.MultiBitmapGetFrameDelay(
                ctypes.c_size_t(self._bitmap),
                delayData,
                delayDataSize,
                ctypes.byref(valueOffset),
            )
            if gdiStatus == 0:
                valueOffset = valueOffset.value
                # the uint of frame delay is 1/100 second
                self._frameDelay = tuple(
                    10 * int.from_bytes(delayData[i : i + 4], byteorder="little")
                    for i in range(valueOffset, valueOffset + 4 * self._frameCount, 4)
                )
    ...
    def ToGifFile(path: str, bitmaps: List[Bitmap], delays: List[int] = None) -> bool:
        ...
        gdiStatus = _DllClient.instance().dll.SaveGif(
            cbitmaps, cdelays, blen, ctypes.c_wchar_p(path)
        )
        return gdiStatus == 0
    ...
    with _ClipboardLock:
        if _OpenClipboard(0):
            if ctypes.windll.user32.IsClipboardFormatAvailable(
                ...
                cbmp = _DllClient.instance().dll.BitmapFromHBITMAP(
