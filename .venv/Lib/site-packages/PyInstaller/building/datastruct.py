...
class TOC(list):
    ...
    def __sub__(self, other):
        ...
        other = TOC(other)
        return TOC(
            [entry for entry in self if unique_name(entry) not in other.filenames]
        )
...
class Target:
    ...
    def __postinit__(self):
        ...
        if last_build == 0:
            logger.info(
                "Building %s because %s is non existent",
    ...
    def _check_guts(self, data, last_build):
        ...
        while stack:
            ...
            if misc.mtime(d) > last_build:
                logger.info(
                    "Building %s because directory %s changed", self.tocbasename, d
                )
                return True
...
def _normalize_toc(
    toc, toc_type_priorities, type_case_normalization_fcn=lambda typecode: False
):
    options_toc = []
    tmp_toc = dict()
    for dest_name, src_name, typecode in toc:
        ...
        if existing_entry is None:
            ...
            if toc_type_priorities.get(typecode, 0) > toc_type_priorities.get(
                existing_typecode, 0
            ):
                tmp_toc[entry_key] = (dest_name, src_name, typecode)
...
def toc_process_symbolic_links(toc):
    ...
    for entry in toc:
        ...
        symlink_entry = _try_preserving_symbolic_link(
            dest_name, src_name, all_dest_files
        )
