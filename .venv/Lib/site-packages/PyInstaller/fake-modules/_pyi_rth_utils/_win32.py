...
kernel32.CloseHandle.argtypes = (ctypes.wintypes.HANDLE,)  # [in] HANDLE hObject
...
def _get_process_sid(token_information_class):
    ...
    try:
        ...
        if ret == 0:
            ...
            raise RuntimeError(
                f"Failed to open process token! Error code: 0x{error_code:X}"
            )
        ...
        if ret == 0:
            error_code = kernel32.GetLastError()
            raise RuntimeError(
                f"Failed to query token information! Error code: 0x{error_code:X}"
            )
        ...
        if token_information_class == TokenUser:
            ...
            raise ValueError(
                f"Unexpected token information class: {token_information_class}"
            )
        ...
        if ret == 0:
            error_code = kernel32.GetLastError()
            raise RuntimeError(
                f"Failed to convert SID to string! Error code: 0x{error_code:X}"
            )
        sid = sid_wstr.value
