...
def binary_dependency_analysis(
    binaries, search_paths=None, symlink_suppression_patterns=None
):
    """
    ...
    for dest_name, src_name, typecode in output_toc:
        ...
        for dep_name, dep_src_path in get_imports(src_name, search_paths):
            ...
            if compat.is_win:
                ...
                dep_src_path.match(pattern) for pattern in symlink_suppression_patterns
            ):
    ...
    for dependency_name, referring_binary in missing_dependencies:
        ...
        if dependency_toc_entry is None:
            ...
            # loop).
            if any(
                dependency_src_path.match(pattern)
                for pattern in symlink_suppression_patterns
            ):
                logger.info(
            ...
            output_toc.append(
                (dependency_basename, str(dependency_dest_path), "SYMLINK")
            )
...
def get_python_library_path():
    ...
    if (compat.is_win or compat.is_cygwin) and os.path.normcase(expected_name).endswith(
        ".dll.a"
    ):
    ...
    if is_shared:
        ...
        for _, lib_path in imported_libraries:
            ...
            if (
                os.path.normcase(os.path.basename(lib_path)) == expected_basename
            ):  # Basename comparison
