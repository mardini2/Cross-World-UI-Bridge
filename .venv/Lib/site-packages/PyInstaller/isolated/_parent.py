...
class Python:
    ...
    def __init__(self, strict_mode=None):
        ...
        self._strict_mode = (
            strict_mode if strict_mode is not None else compat.strict_collect_mode
        )
    ...
    def __enter__(self):
        ...
        if os.name == "nt":
            ...
            # is closed).
            self._write_handle = os.fdopen(
                msvcrt.open_osfhandle(write_to_child, 0), "wb"
            )
            self._read_handle = os.fdopen(
                msvcrt.open_osfhandle(read_from_child, 0), "rb"
            )
        else:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Timed out while waiting for the child process to be killed!"
                )
    ...
    def call(self, function, *args, **kwargs):
        ...
        if self._child is None:
            raise RuntimeError(
                "An isolated.Python object must be used in a 'with' clause."
            )
        ...
        # trace.
        raise RuntimeError(
            f"Child process call to {function.__name__}() failed with:\n" + output
        )
