...
class AppBuilder:
    ...
    def test_source(self, source, *args, **kwargs):
        ...
        scriptfile = gen_sourcefile(
            self._tmp_path, source, kwargs.setdefault("test_id")
        )
        del kwargs["test_id"]
    ...
    def test_script(
        ...
        **kwargs,
        ...
        self._display_message(
            "TEST-SCRIPT", "Build finished, now running executable..."
        )
    ...
    def _find_executables(self, name):
        ...
        if is_darwin:
            ...
            app_bundle_pt = str(
                self._dist_dir / f"{name}.app" / "Contents" / "MacOS" / name
            )
    ...
    def _run_executable_(self, args, exe_path, prog_env, prog_cwd, runtime):
        ...
        process = popen_implementation(
            args, executable=exe_path, env=prog_env, cwd=prog_cwd
        )
        ...
        try:
            ...
            if runtime:
                ...
                self._display_message(
                    "RUN-EXE",
                    f"Process reached expected run-time of {runtime} seconds.",
                )
                retcode = 0
            else:
                ...
                self._display_message(
                    "RUN-EXE",
                    f"Timeout while running executable (timeout: {timeout} seconds)!",
                )
                retcode = 1
            ...
            if psutil is None:
                ...
                try:
                    ...
                    try:
                        ...
                        self._display_message(
                            "RUN-EXE",
                            "Failed to stop the process (or its child process(es))!",
                        )
            else:
                ...
                try:
                    self._display_message(
                        "RUN-EXE",
                        f"Waiting for main process ({process.pid}) to stop...",
                    )
                    stdout, stderr = process.communicate(timeout=5)
    ...
    def _test_building(self, args):
        ...
        if self._is_spec:
            ...
            default_args = [
                ...
                str(self._build_dir),
                "--path",
                str(_get_modules_dir(self._request)),
                "--log-level",
                "INFO",
            ]  # yapf: disable
    ...
    def _examine_executable(self, exe, toc_log):
        ...
        """
        self._display_message(
            "EXAMINE-EXE", f"Matching against TOC log: {str(toc_log)!r}"
        )
        ...
        for pattern in pattern_list:
            for fname in fname_list:
                if re.match(pattern, fname):
                    self._display_message(
                        "EXAMINE-EXE", f"Entry found: {pattern!r} --> {fname!r}"
                    )
                    break
            else:
