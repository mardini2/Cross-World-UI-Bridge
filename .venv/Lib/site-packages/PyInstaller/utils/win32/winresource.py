...
def get_resources(filename, types=None, names=None, languages=None):
    ...
    if "*" not in types:
        available_types = [
            res_type for res_type in available_types if res_type in types
        ]
    ...
    for res_type in available_types:
        ...
        if "*" not in names:
            available_names = [
                res_name for res_name in available_names if res_name in names
            ]
        ...
        for res_name in available_names:
            ...
            if "*" not in languages:
                available_languages = [
                    ...
                    if res_lang in languages
                ]

            for res_lang in available_languages:
                # Read data
                try:
                    data = win32api.LoadResource(
                        module_handle, res_type, res_name, res_lang
                    )
                except pywintypes.error as e:
                    if e.args[0] not in _IGNORE_EXCEPTIONS:
                        raise
...
def add_or_update_resource(filename, data, res_type, names=None, languages=None):
    ...
    resources = resources.get(
        res_type, {}
    )  # This is now a {name: {language: data}} dictionary
...
def copy_resources_from_pe_file(
    filename, src_filename, types=None, names=None, languages=None
):
    """
...
def remove_all_resources(filename):
    ...
    """
    module_handle = win32api.BeginUpdateResource(
        filename, True
    )  # bDeleteExistingResources=True
